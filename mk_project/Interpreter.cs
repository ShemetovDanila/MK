...
/// <summary>
/// Интерпретатор стековой машины. Исполняет ОПС и ловит ошибки времени выполнения.
/// </summary>
public class Interpreter
{
    ...
    private const int MAX_COMMANDS = 1000000;

    /// <summary>
    /// Извлекает числовое значение операнда с проверкой инициализации и границ.
    /// </summary>
    private double GetValue(string token)
    {
        ...
        if (token.Contains("[") && token.Contains("]"))
        {
            ...
            if (idx < 0) throw new Exception($"Отрицательный индекс массива: {idx}");
            if (!arrays.ContainsKey(arrName) || !arrays[arrName].ContainsKey(idx))
                throw new Exception($"Элемент массива '{token}' не инициализирован.");

            return arrays[arrName][idx];
        }

        // Поиск простой переменной
        if (!variables.ContainsKey(token))
            throw new Exception($"Переменная '{token}' используется до инициализации (не было присваивания или read).");
    ...
    public void Execute(List<string> rpn)
    {
        ...
        try
        {
            while (pc < rpn.Count)
            {
                if (++commandsRun > MAX_COMMANDS)
                    throw new Exception("Превышен лимит времени выполнения (бесконечный цикл).");
